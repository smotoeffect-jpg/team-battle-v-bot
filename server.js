...
function getUser(userId){
  ...
  if (!users[userId]){
    users[userId] = {
      userId, team:null, battle:0, incomePerSec:0, lastTick: Date.now(),
      upgrades:{ battery:0 },
      myTeam:{},
      perks:{ vip:false, autoclicker:false, offline:false },
      perkExpiry:{ vip:0, autoclicker:0, offline:0 }
    };
    writeJson(USERS_PATH, users);
  }
  return users[userId];
}
function saveUser(u){ const users = readJson(USERS_PATH); users[u.userId]=u; writeJson(USERS_PATH, users); }
function tick(u){
  const now = Date.now();
  const dt = Math.max(0, Math.floor((now - (u.lastTick||now))/1000));
  if (dt>0){ u.battle += (u.incomePerSec||0)*dt; u.lastTick=now; }
  // expire perks
  ['vip','autoclicker','offline'].forEach(k=>{
    if ((u.perkExpiry && u.perkExpiry[k]) && now > u.perkExpiry[k]){
      u.perks[k]=false;
      if (k==='autoclicker'){ /* no continuous effect here, client may simulate taps elsewhere if you want */ }
      if (k==='offline'){ /* passive handled by tick already */ }
      // VIP could have boosted multipliers—you can implement multipliers client-side if needed.
    }
  });
}

app.get('/api/score', (req,res)=>{ try{ res.json(readJson(SCORES_PATH)); }catch(e){ res.status(500).json({error:'score_fail'});} });

app.get('/api/user/:userId', (req,res)=>{
  const u = getUser(req.params.userId); tick(u); saveUser(u);
...
app.post('/api/user/:userId/team', (req,res)=>{
  ...
  const prev = u.team; u.team=team; saveUser(u); res.json({ok:true, prev, team});
});

app.post('/api/user/:userId/tap', (req,res)=>{
  const u = getUser(req.params.userId); tick(u);
  const mult = (u.perks&&u.perks.vip) ? 1.25 : 1.0;
  const add = Math.max(1, Math.floor(((req.body&&req.body.amount)||1) * mult));
  u.battle += add;
  const scores = readJson(SCORES_PATH); if (u.team){ scores[u.team]=(scores[u.team]||0)+add; writeJson(SCORES_PATH, scores); }
  saveUser(u);
...
app.post('/api/user/:userId/upgrade', (req,res)=>{
  ...
  u.battle -= cost; u.upgrades.battery = lvl+1;
  u.dailyTapCap = (u.dailyTapCap||500)+250;
  saveUser(u); res.json({ok:true, user:u});
});
...
app.post('/api/user/:userId/myteam/buy', (req,res)=>{
  ...
  if (u.battle < cost) return res.status(400).json({error:'insufficient_battle', need:cost});
  u.battle -= cost; entry.level += 1; u.myTeam[itemKey]=entry; u.incomePerSec = (u.incomePerSec||0)+CAT[itemKey].bonus;
  saveUser(u); res.json({ok:true, user:u, cost});
});
...
// --- TB_V15 webhook post-processing (non-invasive) ---
// After your existing successful_payment logic runs, ensure to call this helper with the payload string.
function TB_grantStarsPerkByPayload(payload){
  try{
    if (!payload || typeof payload!=='string') return;
    if (!payload.startsWith('stars:')) return;
    const parts = payload.split(':');
    const key = parts[1]; const userId = parts[2];
    if (!['vip','autoclicker','offline'].includes(key) || !userId) return;
    const u = getUser(userId);
    const now = Date.now();
    const DUR = { vip: 24*7, autoclicker: 24, offline: 72 };
    u.perks[key] = true;
    u.perkExpiry = u.perkExpiry||{};
...
app.post('/api/stars/grant', (req,res)=>{
  ...
  TB_grantStarsPerkByPayload(payload);
  res.json({ok:true});
});
