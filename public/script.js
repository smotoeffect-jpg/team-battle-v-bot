...
/* === Referral Program: Active (3 languages) === */
(function initReferral(){
  try {
    const WebApp = window.Telegram && window.Telegram.WebApp ? window.Telegram.WebApp : null;
    const lang = (localStorage.getItem('tb_lang') || (WebApp?.initDataUnsafe?.user?.language_code||'he').slice(0,2));
    const cur = (['he','en','ar'].includes(lang)?lang:'he');
    const refText = {
    ...
    if (title) title.textContent = refText[cur]?.title || refText.en.title;
    if (btn)   btn.textContent   = refText[cur]?.copy  || refText.en.copy;
    if (input) input.value       = link;
    btn && btn.addEventListener("click", async ()=>{
      try{
        await navigator.clipboard.writeText(link);
        const old = btn.textContent;
        btn.textContent = refText[cur]?.copied || refText.en.copied;
        setTimeout(()=>btn.textContent = old, 1200);
      }catch(e){ console.warn("copy failed", e); }
    });
    document.querySelectorAll(".lang-switch .chip").forEach(chip=>{
      chip.addEventListener("click", ()=>{
        const L = localStorage.getItem('tb_lang') || 'he';
        if (title) title.textContent = refText[L]?.title || refText.en.title;
        if (btn)   btn.textContent   = refText[L]?.copy  || refText.en.copy;
      });
    });
  } catch(e) {
